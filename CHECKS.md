...
| **Looks for** | Constant `args[n]` and bounded slice expressions inside inline, immediately invoked, deferred/goroutine, or exact same-file named Cobra callbacks, cross-checked against `ExactArgs`, `MinimumNArgs`, `RangeArgs`, mechanically proven unshadowed custom validators, path-sensitive reassignment (including `select` receives and `append` lower bounds), and dominating `len(args)` guards |
